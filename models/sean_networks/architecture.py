...
from models.sean_networks.normalization import SEAN, NoiseInjection
...
class SEANResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt, inject_st):
        ...
        self.norm_0 = SEAN(spade_config_str, norm_nc=fin, label_nc=opt.semantic_nc, len_latent=opt.nef * 8,
                           inject_st=inject_st)
        self.noise_1 = NoiseInjection(fmiddle)
        self.norm_1 = SEAN(spade_config_str, norm_nc=fmiddle, label_nc=opt.semantic_nc, len_latent=opt.nef * 8,
                           inject_st=inject_st)
        if self.learned_shortcut:
            self.noise_s = NoiseInjection(fin)
            self.norm_s = SEAN(spade_config_str, norm_nc=fin, label_nc=opt.semantic_nc, len_latent=opt.nef * 8,
                               inject_st=inject_st)
